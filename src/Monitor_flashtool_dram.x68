...
*ACTUALLENGTH    EQU $800000
ACTUALLENGTH    EQU $100000
...
parseLine:
 ...
 .findCommand:
    ...
    beq.w   .deposit
    cmp.b   #'G', d0            * Go (Run) command
    beq.w   .run
    cmp.b   #'I', d0            * Info command
    beq.w   .flashInfo
    cmp.b   #'F', d0            * Flash erase command
    beq.w   .flashErase
...
******************************************
 .flashInfo:
    ...
    * Check Query OK?
    move.w  pDestinationA+($0010<<1), d0    * Address 10h 'QQ'
    bsr.w   printHexWord
    bsr.w   printNewline
    move.w  pDestinationA+($0011<<1), d0    * Address 11h 'RR'
    bsr.w   printHexWord
    bsr.w   printNewline
    move.w  pDestinationA+($0012<<1), d0    * Address 12h 'YY'
    bsr.w   printHexWord
    bsr.w   printNewline
...
******************************************
  .flashErase:
    ...
    lea     msgEraseBanner, A0          * Sending chip erase command sequence
    bsr.w   printString
  ...
  .flashlp01:
    ...
    andi.w  #$4444, d1                  * 01000100 01000100

    move.w  (A1), D0                    * check bits 2 and 6 for toggling
    andi.w  #$4444, d0                  * 01000100 01000100
...
******************************************
  .flashProgram:
    ...
    lea     msgProgMsgSrcAddress, A0      * Display src address
    bsr.w   printString
    move.l   A2, D0
    bsr.w   printHexLong

    lea     msgProgMsgDestAddress, A0    * Display dist address
    bsr.w   printString
    move.l   A1, D0
    bsr.w   printHexLong

    lea     msgProgMsgLength, A0        * Display length
    bsr.w   printString
    move.l  #ACTUALLENGTH, D0
    bsr.w   printHexLong
  ...
  .flashProg1:                          * main loop
    ...
    bsr.w   printNewline                * Display address
    move.l  A1, D0                      * pDestinationA + loop
    add.L   D1, D0
    bsr.w   printHexLong

  .flashProg2:
    cmpi.L  #ACTUALLENGTH, D1           * if(ACTUALLENGTH =< loop)
    bcc.W   .flashProg6
  ...
  .flashProg5:
    move.w  ($0,A1,D1.L), D0            * if (*(pDestinationA + loop) & 0x8080 == (*(pSourceA + loop) & 0x8080))
    andi.w  #$8080, D0
    move.w  ($0,A2,D1.L), D2
    andi.w  #$8080, D2
    cmp.W   D0, D2
...
msgBanner:
    ...
    dc.b       'for MC68EZ328 + Flashtool by kanpapa',CR,LF
    dc.b       '==============================',CR,LF,0
msgHelp:
    dc.b 'Available Commands: ',CR,LF
    dc.b ' (E)xamine   (D)eposit     (G)o      (H)elp',CR,LF
    dc.b 'Flash Commands: ',CR,LF    
    dc.b ' (I)nfo      (F)lash Erase (P)rogram (R)eset',CR,LF,0
msgDepositPrompt:
    dc.b ': ',0
msgPrompt:
    dc.b '> ',0
...
msgProgMsgDestAddress:
    dc.b CR,LF,'Dest address: ',0
msgProgMsgSrcAddress:
    dc.b CR,LF,'Source address: ',0
msgProgMsgLength:
    dc.b CR,LF,'Length: ',0
